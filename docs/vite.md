...
简单记：`vite`在开环境时基于`ESBuild`打包，相比webpack的编译方式，大大提高了项目的启动和热更新速度。
...
3. `webpack`支持的更广。由于 `Vite `基于`ES Module`，所以代码中不可以使用`CommonJs`；`webpack`更多的关注兼容性, 而`Vite` 关注浏览器端的开发体验。`Vite`目前生态还不如 `Webpack`。
