...
`babel`本质上会运行`babel-loader`一个函数，在运行时会匹配到对应的文件，根据`babel.config.js`（.balelrc）的配置（这里会配置相关的`babel-preset-env`,它会告诉babel用什么规则去进行代码转换）去将代码进行一个解析和转换（转换依靠的是babel-core），最终得到目标平台的代码。
